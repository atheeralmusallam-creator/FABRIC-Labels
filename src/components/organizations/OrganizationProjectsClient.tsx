    ...
    <main className="max-w-7xl mx-auto px-6 py-10">
      <div className="flex items-start justify-between gap-4 mb-8">
          ...
          {editing ? (
            <div className="space-y-3 max-w-xl">
              <input
                ...
                className="w-full bg-[var(--bg-secondary)] border border-[var(--border)] focus:border-indigo-500 rounded-lg px-4 py-2.5 text-[var(--text-primary)] text-xl font-bold outline-none"
              />
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={saving}
                  className="bg-gradient-to-r from-emerald-500 to-teal-600  hover:from-emerald-400 hover:to-teal-500  shadow-md shadow-emerald-500/20 disabled:opacity-50 text-[var(--text-primary)] text-sm font-medium px-4 py-2 rounded-lg"
                >
                  {saving ? "Saving..." : "Save"}
                </button>
                ...
                <button
                  ...
                  className="bg-[var(--bg-surface)] hover:bg-[var(--bg-hover)] text-[var(--text-secondary)] text-sm font-medium px-4 py-2 rounded-lg"
                >
                  Cancel
                </button>
              </div>
            ...
            <>
              <div className="flex items-center gap-3">
                <h1 className="text-2xl font-bold text-[var(--text-primary)] truncate">{organization.name}</h1>
              ...
              {organization.description && (
                <p className="text-[var(--text-muted)] text-sm mt-1">{organization.description}</p>
              )}
        ...
        {canManage && !editing && (
          <Link
            ...
            className="flex items-center gap-2 bg-gradient-to-r from-emerald-500 to-teal-600  hover:from-emerald-400 hover:to-teal-500  shadow-md shadow-emerald-500/20 text-[var(--text-primary)] text-sm font-medium px-4 py-2 rounded-lg"
          >
      ...
      <div className="mb-6 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
        <input
          ...
          className="w-full sm:max-w-md bg-[var(--bg-secondary)] border border-[var(--border)] focus:border-indigo-500 rounded-lg px-4 py-2.5 text-[var(--text-primary)] text-sm outline-none"
        />
      ...
      {projects.length === 0 ? (
        <div className="text-center py-24 border border-dashed border-[var(--border)] rounded-xl">
          ...
          <p className="text-[var(--text-secondary)] text-lg font-medium">No projects in this organization yet</p>

          {canManage && (
            <Link
              href={`/organizations/${organization.id}/new-project`}
              className="inline-block mt-6 bg-gradient-to-r from-emerald-500 to-teal-600  hover:from-emerald-400 hover:to-teal-500  shadow-md shadow-emerald-500/20 text-[var(--text-primary)] text-sm font-medium px-5 py-2.5 rounded-lg"
            >
              Create Project
            </Link>
          )}
        ...
        <div className="text-center py-20 border border-dashed border-[var(--border)] rounded-xl">
          <p className="text-[var(--text-secondary)] text-lg font-medium">No projects match this filter</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredProjects.map((project) => {
            ...
            return (
              <div
                key={project.id}
                className={`relative group bg-[var(--bg-secondary)] border border-[var(--border)] hover:border-indigo-500/50 rounded-xl transition-all hover:bg-[var(--bg-surface)] ${
                ...
                <Link href={`/projects/${project.id}`} className="block p-5">
                  <div className="flex items-start justify-between mb-3 pr-10">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <h2 className="text-sm font-semibold text-[var(--text-primary)] group-hover:text-indigo-300 truncate">
                        {project.name}
                      </h2>
                    </div>
                ...
                {canManage && (
                  ...
                  >
                    <button
                      ...
                      className="w-8 h-8 rounded-lg bg-[var(--bg-surface)] border border-[var(--border)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:border-indigo-500/50"
                    >
                      ...
                    </button>

                    {openMenuId === project.id && (
                      <div className="absolute right-0 mt-2 w-44 bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg shadow-2xl overflow-hidden z-[9999]">
                        ...
                        <a
                          ...
                          download
                          className="block w-full text-left px-3 py-2 text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-surface)] hover:text-[var(--text-primary)]"
                        >
                          Export IAA
                        </a>
